...
const hasLibFile = (() => {
  ...
  return function hasLibFile(fromFilePath) {
    ...
    try {
      result =
        fs.readdirSync(folderPath).find((f) => f.endsWith('.lib')).length > 0;
    } finally {
  ...
  return {
    Program() {
      ...
      if (
        onlyOneBlock &&
        firstScope &&
        firstScope.type === 'function' &&
        firstScope.block.id &&
        firstScope.block.id.name
      ) {
        couldBeAnAuraService = hasLibFile(context.getFilename());
      }
    },
...
module.exports = {
  meta: {
    messages: {
      invalidName:
        "The Aura service must be named '{{expectedServiceName}}', got '{{actualServiceName}}'",
    },
  },
  schema: [],
  rules: {
