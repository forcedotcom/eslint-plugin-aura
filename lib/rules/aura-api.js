      ...
      if (ns.length > 0) {
        ...
        for (let i = 0; i < ns.length; ++i) {
          ...
          if (identifier.type !== 'Identifier') {
            context.report({node, messageId: 'mustUseDotNotation', data: { name: identifier.name }})
            return
          }
          ...
          if (typeof api !== 'object') {
            context.report({node, messageId: 'invalidApi', data: { name: token }})
            return
          }
...
module.exports = {
  meta: {
    messages: {
      mustUseDotNotation: "Invalid Aura API '{{name}}', use dot notation instead",
